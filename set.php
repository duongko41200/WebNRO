...
if ($_user != null) {
	...
	$_referralCode= $user_arr['referral_code'];
	$_coin = $user_arr['vnd'];
	$_vnd_reward = $user_arr['reward'];
	$_coin_total = $user_arr['tongnap'];
	$_coin_affilate = $user_arr['coin_affilate'];
	$_status = $user_arr['active'];
	$_status_game = $user_arr['ban'];
	$_ip_address = $user_arr['ip_address'];
	switch ($_status) {
		case '-1':
